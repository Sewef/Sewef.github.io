...
const getStoredTheme=()=>{const theme=localStorage.getItem('theme')
...
const setTheme=theme=>{if(theme==='auto'&&window.matchMedia('(prefers-color-scheme: dark)').matches){document.documentElement.setAttribute('data-bs-theme','dark')}else if(theme==='auto'){document.documentElement.setAttribute('data-bs-theme','light')}else{document.documentElement.setAttribute('data-bs-theme',theme)}}
...
const initThemeSwitcher=()=>{showActiveTheme(getPreferredTheme())
...
if(document.readyState==='loading'){window.addEventListener('DOMContentLoaded',initThemeSwitcher)}else{initThemeSwitcher()}
